...
if __name__ == "__main__":
    ...
    clf = RandomForestClassifier(random_state=randomSeed, n_jobs=CPU_num, **params).fit(x_test_RF6_list[0],
                                                                                        y_test_list[0])
    ...
    for i in range(5):
        ...
        if not i:
            ax1[i].plot(fpr, tpr, color=palette[0], linestyle='-',
                        label='%s AUC: %.2f (%.2f, %.2f)' % (LLRmodelNA[0:4], AUC, AUC_05, AUC_95))
        else:
        ...
        AUC, _, AUC_05, AUC_95 = AUC_with95CI_calculator(y_true, y_pred)
        if not i:
            ax1[i].plot(fpr, tpr, color=palette[2], linestyle='-',
                        label='RF6 AUC: %.2f (%.2f, %.2f)' % (AUC, AUC_05, AUC_95))
        else:
        ...
        AUC, _, AUC_05, AUC_95 = AUC_with95CI_calculator(y_true, y_pred)
        if not i:
            ax1[i].plot(fpr, tpr, color=palette[3], linestyle='-',
                        label='TMB AUC: %.2f (%.2f, %.2f)' % (AUC, AUC_05, AUC_95))
    ...
    ax1 = [0] * 8
    fig1, ((ax1[0], ax1[1]), (ax1[2], ax1[3]), (ax1[4], ax1[5]), (ax1[6], ax1[7])) = plt.subplots(4, 2,
                                                                                                  figsize=(6.5, 6.5))
    ...
    LLRx_data = [AUPRC_LLRx, OddsRatio_LLRx, Accuracy_LLRx, F1_LLRx, PPV_LLRx, NPV_LLRx, Specificity_LLRx,
                 Sensitivity_LLRx]
